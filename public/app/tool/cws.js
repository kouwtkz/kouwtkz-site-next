...
if (cws.update) {
  if (typeof (Object.keys) === "undefined") {
    Object.keys = function (o) {
      var k = [];
      for (var i in o) if (o.hasOwnProperty(i)) k.push(i);
      return k;
    };
    Array.prototype.indexOf = function (e) {
      ...
      if (f < 0) f += l;
      for (; f < l; f++) if (f in this && this[f] === e) return f;
      return -1;
    };
  }
  ...
  cws.get.delimiter = function (re) {
    ...
    if (ret = re.match(/^([^a-zA-Z0-9\\]).*([^a-zA-Z0-9\\])[a-zA-Z]*$/)) {
      ...
      if (
        braceDelimiters[leftDlmt] && rightDlmt === braceDelimiters[leftDlmt] ||
        leftDlmt === rightDlmt
      ) {
        delimiter = leftDlmt;
      }
    ...
    var keywords = cws.get.split_space(keyword).map(function (v) {
      this.escape = function (v) {
        return (" " + v).replace(/\\\\/g, "\\\?")
          ...
          .replace(/\|\|/g, " OR ").replace(/\&\&/g, " AND ").replace(
            /(\s+)\-/g,
            " NOT ",
          ).replace(/(\s+)\#/g, " TAG ")
          .replace(/^\s+/, "");
        ...
        switch (re) {
          ...
          default:
            ...
            if (typeof (add_val) === "string" && w_mode) {
              ...
              if (hw !== fw) {
                add_val = [
                  new hook_class(hw, "", false, hook_tag),
                  new hook_class(fw, "|", false, hook_tag),
                ];
              }
            }
            if (hook_mode === "") {
              ...
              if (add_val !== "") {
                hook_value.push(
                  new hook_class(add_val, hook_mode, hook_not, hook_tag),
                );
              }
              hook_mode = "";
              hook_not = false;
  ...
  cws.get.search = function (subject, keyword) {
    var str = " " +
      subject.replace(/[\[](.*)[\]]/, " [ $1 ] ").replace("/[\#\s]+/g", " ") +
      " ";
    var result = true;
    var keywords = keyword;
    if (!Array.isArray(keywords)) {
      keywords = cws.get.hook_search(keywords, w_mode);
    }
    ...
    var s_search = function (value) {
      ...
      value.filter(function (hook) {
        ...
        m_result = (function (a, b) {
          return Boolean((a & !b) | (!a & b));
        })(m_result, hook.mode_not);
        if (or_trigger) {
  ...
  cws.to.merge = function (obj_a, obj_b, null_blank) {
    if (
      typeof (obj_b) !== "undefined" && obj_b === null &&
      Boolean(cws.check.def(null_blank, true))
    ) return new Object();
    ...
    if (typeof (data) === "object") {
      if (typeof (data.elements) !== "undefined") {
        ...
        for (var i = 0; i < elem.length; i++) {
          name = elem[i].name;
          if (
            typeof (elem[i].files) === "undefined" || elem[i].files !== null
          ) {
            value = elem[i].files;
          } else {
            value = elem[i].value;
  ...
  cws.to.querystr = function (data, urlencoded, no_value_equal, no_name_send) {
    ...
    if (urlencoded) {
      retvar = encodeURI(retvar.replace(/\%(\d+)/g, "?$1?")).replace(
        /\?(\d+)\?/g,
        "%$1",
      );
    }
  ...
  cws.to.setQuery = function (array_list, path) {
    ...
    var q = cws.to.querystr(
      cws.to.merge(cws.get.request(path), array_list),
      true,
    );
    return _path + ((q === "") ? "" : "?") + q;
  };
  cws.to.form_append = function (request, formdata_obj, merge_mode) {
    ...
    merge_mode = typeof (formdata_obj.has) === "undefined"
      ? false
      : cws.check.nullvar(merge_mode, false);
  ...
  cws.to.asctochar = function (str, decode, plus_to_space) {
    ...
    for (var i = 0; i < cws.v.urlrg.length; i++) {
      var chkstr = new RegExp(
        "\\%" + cws.v.urlrg[i].charCodeAt().toString(16),
        "gi",
      );
  ...
  cws.ajax.onload = function (x, e) {
    console.log(e);
  };
  cws.ajax.onerror = function (x, e) {
    console.log("error");
    console.log(e);
  };
  cws.ajax.onbusy = function (x, e) {
    console.log(e);
  };
  cws.ajax.oncatch = function (x, e) {
  ...
  cws.ajax.run = function (args) {
    ...
    try {
      ...
      if (
        tmp_type === "[object HTMLFormElement]" ||
        tmp_type === "[object FormData]"
      ) form = tmp;
      if (form !== null) {
        if (formdata_check) {
      ...
      query = cws.to.merge(
        ...
        cws.to.merge(
          cws.check.key(args, "request", new Object()),
          cws.check.key(args, "query", new Object()),
        ),
      );
      href = href.replace(/\?.*$/, "");
      ...
      if (Object.prototype.toString.call(onload) !== "[object Function]") {
        onload = cws.ajax.onload;
      }
      if (opt & 1) {
        query["refpath"] = cws.check.key(args, "refpath", location.pathname)
          .toString();
      }
      if (method === "GET") {
  ...
  cws.dom.setupFocus = function (element) {
    if (Object.prototype.toString.call(element).match("HTML")) {
      element.insertAdjacentHTML(
        "beforebegin",
        '<a id="__on_focus_before_dammy_a" href></a>',
      );
  ...
  cws.dom.removeChildren = function (elm) {
  ...
  cws.dom.autoreload = function (reload_target) {
    ...
    if (cws.check.exists(cws.v.request, reload_target)) {
      var interval = parseInt(
        parseFloat(cws.check.key(cws.v.request, reload_target, 0)) * 1000,
      );
  ...
  cws.jump.location = function (href, auto_back) {
    ...
    if (
      auto_back && window.location.href !== href &&
